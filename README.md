...
Implementation of the improved elliptic curve scalar multiplication algorithm required some reorganisation of the `bitcoin_math` source code. First, I renamed the `PT` struct, which holds the xy coordinates of an affine point as a pair of `bnz_t` variables, to `APT` to reflect the need to differentiate it from the new `JPT` struct which holds the xyz Jacobian coordinates required by the efficient algorithms as three `bnz_t` variables. The pre-calculated values of the 256 Secp256k1 generator point doublings modulo the Secp256k1 curve's prime are now stored as a static array of `APT` structs within the SECP256K1 struct. The array is populated during execution of the `secp256k1_init()` function from an external binary file (`dat.bin`). The file comtains 16,384 bytes representing the 32 byte `digits` arrays (in little endian order) of the 512 `bnz_t` structs of the 256 ACT structs that make up the array of 256 generator point doublings modulo the Secp256k1 curve's prime.
